...
for i, anoFilePath in enumerate(anoFilePaths):
    ...
    if imgFileName is None :
        ErrorEnd('Error | XMLファイル内で annotation/size/height が見つかりませんでした' + anoFilePath)
    ...
    if(os.path.splitext(os.path.basename(anoFilePath))[0] != os.path.splitext(os.path.basename(imgFileName))[0]):
        ErrorEnd('Error | XMLファイル名と XMLファイル内に書き込まれている画像ファイルの名称が一致していません' + anoFilePath)
    ...
    for j, obj in enumerate(anoRoot.iter('object')):
        ...
        if label is None :
            ErrorEnd('Error | XMLファイル内に object/name が存在しないオブジェクトが存在します' + anoFilePath)
        if xMin is None :
            ErrorEnd('Error | XMLファイル内に object/bndbox/xmin が存在しないオブジェクトが存在します' + anoFilePath)
        if yMin is None :
            ErrorEnd('Error | XMLファイル内に object/bndbox/ymin が存在しないオブジェクトが存在します' + anoFilePath)
        if xMax is None :
            ErrorEnd('Error | XMLファイル内に object/bndbox/xmax が存在しないオブジェクトが存在します' + anoFilePath)
        if yMax is None :
            ErrorEnd('Error | XMLファイル内に object/bndbox/ymax が存在しないオブジェクトが存在します' + anoFilePath)
